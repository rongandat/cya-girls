...
class Mail {
    ...
    public $crlf = "\r\n";
    public $verp = false;
    public $parameter = '-freturn@yourdomain.com';
    ...
    public function addSubject($subject, $replace = array()) {
        $this->subject = $this->replaceContent($subject, $replace);
    }

    public function addText($text, $replace = array()) {
        $this->text = $this->replaceContent($text, $replace);
    }

    public function addHtml($html, $replace = array()) {
        $this->html = $this->replaceContent($html, $replace);
    }

    public function replaceContent($content, $replaces = array()) {
    ...
    public function send() {
        ...
        if ($this->protocol != 'mail') {
            $header .= 'To: ' . $this->toName . '<' . $to . '>' . $this->newline;
        ...
        if (!empty($this->cc)) {
            $header .= 'CC: ' . $this->ccName . '<' . $this->cc . '>' . $this->newline;
        }
        if (!empty($this->bcc)) {
            $header .= 'BCC: ' . $this->bccName . '<' . $this->bcc . '>' . $this->newline;
        }
        $header .= 'Reply-To: ' . '=?UTF-8?B?' . base64_encode($this->sender) . '?=' . '<' . $this->from . '>' . $this->newline;
        $header .= 'Return-Path: ' . $this->from . $this->newline;
        $header .= 'X-Sender: ' . $this->sender . $this->newline;
